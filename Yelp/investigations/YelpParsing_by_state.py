...
reviews = sqlContext.sql("select B.state as state, \
                        R.business_id as id, \
                        R.text as text, \
                        R.stars as stars, \
                        R.date as date \
from review R join business B on R.business_id = B.business_id \
where R.text is not null and B.text is not null")
...
columns = ['state','business_id','clean_text', 'tot_words', 'bigrams','trigrams','stars','date']
...
# Save it as a table
df.registerTempTable("dfState")
sqlContext.sql("drop table if exists resultbyState")
sqlContext.sql("CREATE TABLE resultbyState AS SELECT * FROM dfState")